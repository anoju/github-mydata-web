<template>
  <kb-page page-title="내 자산 Answer">
    <kb-page-body>
      <div class="my_qna_answer">
        <h1 class="tit">내가 지금 보유하고 있는 주식 중 <strong>보유 비중이 가장 큰 주식</strong>은 무엇일까?</h1>
        <div class="chart_wrap mg_t40">
          <img src="~@/assets/images/temp/chart5.png" alt="" />
        </div>
        <div class="round_txt_box mg_t32" v-scroll-animate="'zoomIn'">
          <strong>하이트진로홀딩스 종목을<br>가장 많이 보유하고 있어요!</strong>
        </div>
        <div class="fz_12 fc_666 t_center mg_t16">2020/04/14 ~ 2021/04/14의 분석</div>
      </div>
      <kb-button-wrap bottom-fixed>
        <kb-button
          line
        >
          목록보기
        </kb-button>
      </kb-button-wrap>
    </kb-page-body>
  </kb-page>
</template>
<script>
export default {
  name: 'TO01D004',
  data() {
    return {
    };
  },
  methods: {
  },
};
</script>
