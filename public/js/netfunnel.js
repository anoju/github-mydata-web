...
NetFunnel.TS_HOST        	= 'flowcontrol.kbsec.com';		// Default TS host
NetFunnel.TS_PORT			= 443;				// Default TS port
NetFunnel.TS_PROTO			= 'https';			// Default TS protocol [http|https]
NetFunnel.TS_QUERY			= 'ts.wseq';		// Default request query
NetFunnel.TS_SERVICE_ID		= 'service_1';		// Default TS Service id
NetFunnel.TS_ACTION_ID		= 'd_md01';			// Default TS Action id
...
//Server Block
NetFunnel.TS_BLOCK_MSG			= '일시적으로 서비스가 차단 되었습니다.';// Server Block시 팝업에 표시할 문구
...
NetFunnel.Util = {
	...
	getTimeStr:function(inTime,format,delimiter,force){
		...
		var m=false;
		var s=false;
		var match_strs=format.match(/%[-]*[0-9]*[H|M|S]/g);
		for(j=0; j < match_strs.length; j++){
			match_str = match_strs[j];
			item=match_str.charAt(match_str.length-1);
			if(item == "H") h = true;
			if(item == "M") m = true;
			if(item == "S") s = true;
		}
		...
		// Format 치환
		var result="";
		var format_arr=format.split(delimiter);

		for(var i=0; i < format_arr.length; i++){
			var t_str=format_arr[i];
			match_strs=t_str.match(/%[-]*[0-9]*[H|M|S]/g);
			var print_match=true;

			for(j=0; match_strs && j < match_strs.length; j++){
				match_str = match_strs[j];
				...
				if(match_str.length > 2){
					var cnt="";
					var start=true;
					for(var k=1; k < match_str.length-1; k++){
						var ss = match_str[k];
					...
					pad_size=parseInt(cnt, 10);
				}

				var number_str="";
				if(item=="H"){
					if(tHour==0) print_match=false;
					number_str=""+tHour;
				}else if(item=="M"){
					if(tMin==0) print_match=false;
					number_str=""+tMin;
				}else if(item=="S"){
					number_str=""+tSec;
				}
				...
				if(pad){
					...
					var pad_diff=pad_size-number_str.length;

					for(var l=0; l < pad_diff; l++){
						rep_str += pad_str;
					}
				...
				t_str = t_str.replace(match_str,rep_str);
			}

			if(force==true || print_match==true){
				if(result.length > 0){
					result = result+delimiter+t_str;
				}else{
					result = t_str;
				}
			}
...
NetFunnel.Storage.prototype.getItem  = function(key, storage_only)
{
	var retval=false;
	try{
		if(this.html5Support){
			retval = this.getStorage().getItem(key);
			if(!retval && (storage_only == undefined || storage_only == false)){
...
NetFunnel.Storage.prototype.removeItem = function(key, storage_only)
{
	try{
		if(this.html5Support){
			this.getStorage().removeItem(key);
			if(storage_only == undefined || storage_only == false){
    ...
    if(NetFunnel.gLastData && NetFunnel.gLastData.time_left >= 0)
    {
		...
		if(NetFunnel.gPop){
			...
			if (tNext){
				...
				else if(shownext_limit > 0 && NetFunnel.gLastData.nnext > shownext_limit){
				...
				if(showtime_limit > 0 && NetFunnel.gLastData.real_time_left > showtime_limit){
					...
					if(tformat.length > 0){
						...
						tTime.innerHTML=NetFunnel.Util.getTimeStr(NetFunnel.gLastData.real_time_left
							,tformat_arr[0],tformat_arr[1],eval(tformat_arr[2])
						);
					}else{
						tTime.innerHTML=NetFunnel.Util.getTimeStr(NetFunnel.gLastData.real_time_left);
					}
			...
			<div id="NetFunnel_Skin_Top" style="background-color:#ffffff;border:1px solid #9ab6c4;overflow:hidden;width:250px;-moz-border-radius: 5px; -webkit-border-radius: 5px; -khtml-border-radius: 5px; border-radius: 5px;" > \
				<div style="background-color:#ffffff;border:6px solid #eaeff3;-moz-border-radius: 5px; -webkit-border-radius: 5px; -khtml-border-radius: 5px; border-radius: 5px;"> \
					...
					<div style="padding-top:5px;padding-left:5px;padding-right:5px"> \
						<div style="text-align:center;font-size:12pt;color:#001f6c;height:22px"><b><span style="color:#013dc1">Waiting</span> for Service Access.</b></div> \
						<div style="text-align:right;font-size:9pt;color:#4d4b4c;padding-top:4px;height:17px;" ><b>Expected Wait Time : <span id="NetFunnel_Loading_Popup_TimeLeft" class="%02H:%02M:%02S sec^:^false"></span></b></div> \
						...
						<div style="background-color:#ededed;padding-bottom:8px;overflow:hidden;width:228px"> \
							<div style="padding-left:5px"> \
								<div style="text-align:center;font-size:8pt;color:#4d4b4c;padding:3px;padding-top:10px;padding-bottom:10px;height:10px">waiting <b><span style="color:#2a509b"><span id="NetFunnel_Loading_Popup_Count" class="'+NetFunnel.TS_LIMIT_TEXT+'"></span></span></b> persons, next waiting <b><span style="color:#2a509b"><span id="NetFunnel_Loading_Popup_NextCnt" class="'+NetFunnel.TS_LIMIT_TEXT+'"></span></span></b> persons</div> \
								<div style="font-family:arial;text-align:left;font-size:8pt;color:#4d4b4c;padding:3px;height:10px">Many people are connecting to this service now, </div> \
								<div style="font-family:arial;text-align:left;font-size:8pt;color:#4d4b4c;padding:3px;height:10px;">and you will be connected automatically if waiting for a moment. \
								<div style="text-align:center;font-size:9pt;color:#2a509b;padding-top:10px;"> \
									<b>[<span id="NetFunnel_Countdown_Stop" style="cursor:pointer">Stop</span>]</b> \
								</div> \
							</div> \
						</div> \
...
NetFunnel.tstr +=	'</span></div> \
			<div style="padding-top:0px;padding-left:25px;padding-right:25px"> \
				<div style="box-sizing:initial;text-align:left;font-size:12pt;color:#001f6c;height:22px"><b> <span style="color:#013dc1">Waiting</span> for Service Access.</b></div> \
				<div style="box-sizing:initial;text-align:right;font-size:9pt;color:#4d4b4c;padding-top:4px;height:17px" ><b>Expected Wait Time :  <span id="NetFunnel_Loading_Popup_TimeLeft" class="%02H:%02M:%02S sec^:^false"></span></b></div> \
				...
				<div style="background-color:#ededed;width:400px;padding-bottom:8px;overflow:hidden"> \
					<div style="padding-left:5px"> \
						<div style="box-sizing:initial;text-align:left;font-size:8pt;color:#4d4b4c;padding:3px;padding-top:10px;height:10px">waiting <b><span style="color:#2a509b"><span id="NetFunnel_Loading_Popup_Count" class="'+NetFunnel.TS_LIMIT_TEXT+'"></span></span></b> persons, next waiting  <b><span style="color:#2a509b"><span id="NetFunnel_Loading_Popup_NextCnt" class="'+NetFunnel.TS_LIMIT_TEXT+'"></span></span></b> persons</div> \
						<div style="box-sizing:initial;font-family:arial;text-align:left;font-size:8pt;color:#4d4b4c;padding:3px;height:10px">Many people are connecting to this service now, </div> \
						<div style="box-sizing:initial;font-family:arial;text-align:left;font-size:8pt;color:#4d4b4c;padding:3px;height:10px;">and you will be connected automatically if waiting for a moment. </div> \
						<div style="box-sizing:initial;font-family:arial;text-align:center;font-size:9pt;color:#2a509b;padding-top:10px;"> \
							<b>[<span id="NetFunnel_Countdown_Stop" style="cursor:pointer">Stop</span>]</b> \
						</div> \
					</div> \
				</div> \
 ...
 * Modal Popup Window
 */
NetFunnel.Popup = function(content,top,left,winobj,waiting,obj_id,zindex) {
	...
	if(!tObj || NetFunnel.SkinUtil.prevID != NetFunnel.gSkinId)
	{
		...
		if(tObj_ct_busy){
			var t_config = { count:50, interval:50  };
			var busyWidth = parseInt(tObj_ct_busy.style.width, 10);
			var busyHeight = parseInt(tObj_ct_busy.style.height, 10);

			if(!isNaN(busyWidth)){
				t_config.width = busyWidth;
			}
			if(!isNaN(busyHeight)){
				t_config.height = busyHeight;
			}

			var busybox = new NetFunnel.ProgressBar(tObj_ct_busy,t_config,this._mDoc);
			busybox.show();
...
NetFunnel.TsClient.prototype.getTidChkEnter = function(user_id,user_tid,first,total_wait){
	NetFunnel.gPrevWaitTime = -1;
	if(NetFunnel.TS_BYPASS == true) { this.fireEvent(null,this,'onSuccess',{rtype:this._mReqType,code:NetFunnel.kSuccess,data:{},next:this.next.success}); return true; }
	...
	NetFunnel.gTotWait = (total_wait == undefined || isNaN(total_wait))?-1:total_wait;
	NetFunnel.retryData = null;
	var ret = (this.mConfig["mp_use"] == true)?NetFunnel.MProtect():0;
...
//	try{
		...
		if(typeof data != "undefined" && data.constructor == Object){
			...
			var retval = new NetFunnel.RetVal(result);
			var ret_code = retval.getRetCode();
			var ret_type = retval.getReqType();
			if(ret_code != NetFunnel.kSuccess && ret_code != NetFunnel.kTsBypass
				&& !(ret_type == NetFunnel.RTYPE_ALIVE_NOTICE && ret_code == NetFunnel.kContinue))
			{
				var t_retval = new NetFunnel.RetVal(NetFunnel.RTYPE_SET_COMPLETE+":200:msg=\"Success\"");
				NetFunnel.gControl._showResultSetComplete(t_retval);
				return true;
			}
